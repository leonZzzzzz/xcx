    ...
    <div class="detailWrapper" ref="detailWrapper" v-if="showDetail">
      <div class="foodDetail">
        ...
        <div class="info">
          ...
          <div class="desc">
            <span class="food-name" style="color: #000000;font-weight: bold;font-size: 0.45rem">{{foodDetail.foodName}}</span>
          </div>
          <div class="price">
            <span class="unit">￥</span>
            <span class="oldPrice" style="color: #FF3A43;">{{food.goodsFoodSpecList[0].nowPrice}}</span>
          </div>
          <div class="food-energy">
            <ul class="energy-ul" style="display: flex;justify-content: space-between;">
              <li>
                <div class="energy-num">{{energy}}千卡</div>
                <div class="energy-name" style="color:#8E8E8E ">热量</div>
              </li>
              <li>
                <div class="energy-num">{{cho}}g</div>
                <div class="energy-name" style="color:#8E8E8E ">碳水化合物</div>
              </li>
              <li>
                <div class="energy-num">{{pro}}g</div>
                <div class="energy-name" style="color:#8E8E8E ">蛋白质</div>
              </li>
              <li>
                <div class="energy-num">{{fat}}g</div>
          ...
          <!--<div class="shopCart">-->
            ...
            <!--</transition>-->
          <!--</div>-->
<!--          <CartControl ref="carContal" :food="food" @on-add="addFood" @on-sub="subFood" @ShowSpec="ShowSpec" :isAll="true" :selectFoodsItemProp="selectFoodsItemProp"></CartControl>-->
        </div>
        <div class="desc">
        ...
        <div class="stock">
          <div class="stock-title">原材料</div>
           <ul class="stock-title-ul">
             <li v-for="item in rideDetails.storageMaterialVO" :key="item.id">{{item.storageMaterialName}}<span>{{item.materialQuantity}}(g)</span></li>
           </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    listByGoodsFoodId () {
      GoodsAPI.listByGoodsFoodId({ goodsFoodId: this.food.id }, res => {
        this.eveList = res
      })
    },
    // 获取原材料
    ...
    showToggle (food) {
      ...
      GoodsAPI.foodDetail({ id: this.food.id }, res => {
        this.foodDetail = res
