<template>
  <div class="">
    <div class="year_str" v-if="showYear">我的{{yearMonthStr}}</div>
      ...
      <div
        ...
        <template v-for="(item, index) in dates">
          <div class="slider"
               ...
               @transitionend="onTransitionEnd(index)" :key="item.id">
            <div class="day" v-for="day in getDaies(item.date)" :key="day.id">
              <div
              ...
              >
                我的{{day.week}}<br><strong>{{day.date.split('-')[2]}}</strong>
              </div>
            </div>
          </div>
        </template>
